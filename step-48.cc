 ...
 *
 * This file includes annotations for use on a GPU (Steve DeWitt, 2019)
 */
...
// GPU vs CPU typedefs
#if USE_GPU
typedef dealii::LinearAlgebra::CUDAWrappers::Vector<double> VectorType;
typedef dealii::CUDAWrappers::MatrixFree<DIM, double> MatrixFreeType;
typedef dealii::CUDAWrappers::FEEvaluation<DIM, DEGREE> FEEvaluationType;
#else
typedef dealii::LinearAlgebra::distributed::Vector<double> VectorType;
typedef dealii::MatrixFree<DIM, double> MatrixFreeType;
...
namespace Step48
{
  ...
  {
  public:
    SineGordonOperation(const MatrixFreeType &data_in,
                        const double                   time_step);

    void apply(VectorType &dst,
    ...
    const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    ...
    FEEvaluationType current(data), old(data);
  ...
  void SineGordonProblem<dim>::make_grid_and_dofs()
  {
    ...
    // NOTE: The treatment of AdditionalData is different for the GPU version
    // of MatrixFree, so we have to do things a little differently here
    #if USE_GPU
